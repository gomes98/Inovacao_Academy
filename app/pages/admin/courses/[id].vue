        ...
        <div v-for="mod in modules" :key="mod.id" class="glass-card rounded-3xl border border-white/10 bg-white/[0.02] backdrop-blur-xl overflow-hidden group/mod">
          ...
          <div class="divide-y divide-white/5">
            ...
            <div v-for="content in mod.contents" :key="content.id" class="p-4 flex items-center gap-4 hover:bg-white/[0.02] transition-colors group/item">
              <div class="w-10 h-10 rounded-xl bg-white/5 flex items-center justify-center text-gray-400 border border-white/5 overflow-hidden flex-shrink-0 relative">
                <template v-if="content.content_type === 'video' && (content as any).video_url">
                  <img
                    :src="((content as any).video_url as string).replace(/\.[^.]+$/, '.jpg')"
                ...
                <svg v-else-if="content.content_type === 'video'" xmlns="http://www.w3.org/2000/svg" width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><polygon points="5 3 19 12 5 21 5 3"/></svg>
