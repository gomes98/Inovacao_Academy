...
// 5. Busca o progresso do usuário — primeiro o conteúdo atual, depois a playlist inteira
const { data: currentProgress, refresh: refreshCurrentProgress } = await useAsyncData(() => `current-progress-${user.value?.id}-${contentId.value}`, async () => {
...
const { data: playlistProgress, refresh: refreshPlaylistProgress } = await useAsyncData(() => `playlist-progress-${user.value?.id}-${content.value?.module_id}`, async () => {
  const userId = user.value?.id
...
const userProgress = computed(() => playlistProgress.value ?? [])

async function refreshProgress() {
  await Promise.all([refreshCurrentProgress(), refreshPlaylistProgress()])
}

const isCompleted = computed(() => currentProgress.value ?? userProgress.value?.includes(contentId.value))
          ...
          <div class="space-y-12">
            <div class="max-w-4xl">
              <div class="flex flex-col md:flex-row md:items-start gap-4 mb-8">
                <h1 class="text-3xl lg:text-4xl font-bold bg-gradient-to-r from-white to-gray-400 bg-clip-text text-transparent flex-1">{{ content?.title }}</h1>

                <div
                  v-if="isCompleted"
                  class="flex-shrink-0 flex items-center gap-2 px-4 py-2 rounded-2xl bg-emerald-500/10 text-emerald-400 border border-emerald-500/20 text-xs font-bold uppercase tracking-widest"
                >
